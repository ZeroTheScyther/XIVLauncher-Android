...
public class PresentExtension implements Extension {
    /** xla: per-frame hook for the performance overlay's FPS counter. */
    public interface PresentListener { void onPresent(int windowId); }

    public static volatile PresentListener presentListener;
    ...
    private void presentPixmap(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        // xla: one call per presented frame (the Vulkan path below bypasses WindowManager's content listeners).
        PresentListener listener = presentListener;
        if (listener != null) listener.onPresent(windowId);
