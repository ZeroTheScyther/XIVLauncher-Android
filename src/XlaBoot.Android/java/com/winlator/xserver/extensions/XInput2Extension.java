...
public class XInput2Extension implements Extension {
    ...
    private void selectEvents(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        for (int i = 0; i < numMasks; i++) {
            ...
            selections.add(sel);
            xla.MouseTrace.log("XISelectEvents window=0x" + Integer.toHexString(windowId) + " device=" + deviceId
                    + " rawMotion=" + mask.isSet(XI_RawMotion_MASK) + " rawButtons=" + mask.isSet(XI_RawButtonPress_MASK));
        }
