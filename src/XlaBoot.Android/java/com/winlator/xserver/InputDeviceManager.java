...
public class InputDeviceManager implements Pointer.OnPointerMotionListener, Keyboard.OnKeyboardListener, WindowManager.OnWindowModificationListener, XResourceManager.OnResourceLifecycleListener {
    ...
    public void onPointerButtonRelease(Pointer.Button button) {
        if (xServer.isRelativeMouseMovement()) {
            // A wheel notch is a button press+release on the X side but a single event on the Windows side. The press
            // already sent WHEEL with the delta; sending WHEEL again for the release is a second wheel event of 0,
            // which DirectInput reports as the wheel's latest movement and the game reads as no scroll at all.
