...
public abstract class WindowRequests {
    ...
    public static void warpPointer(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException, XRequestError {
        ...
        if (srcWindow != null) {
            ...
            if (!isContained) {
                xla.MouseTrace.warp(false, client.xServer.pointer.getX(), client.xServer.pointer.getY(), dstX, dstY);
                return;
            }
        }

        int fromX = client.xServer.pointer.getX(), fromY = client.xServer.pointer.getY();
        if (dstWindow == null) {
        ...
        xla.MouseTrace.warp(true, fromX, fromY, client.xServer.pointer.getX(), client.xServer.pointer.getY());
    }
